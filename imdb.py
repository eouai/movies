...
for title in tqdm.tqdm(missing.keys()):
	try:
		...
		title_split += '&ref_=nv_sr_sm'
		this_page = URL + title_split
		...
		for div in divs:
			if div.attrs.get('class') is not None and 'ipc-metadata-list-summary-item__tc' in div.attrs.get('class'):
				a = div.find('a')
				year = div.find('span').text
				url = imdb_base_url + a.get('href')
				if year == missing[title]:
					...
					print('Mismatched searched result:', title, 'expected year:', missing[title], 'year returned:', year)
